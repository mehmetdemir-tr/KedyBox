...
def extract():
    ...
    # ret1 = os.system(f"{unzip_loc} -oqq paticommands.zip -d pati-commands/")
...
def compile():
    ...
    if not dosyalar:
        ...
        sys.exit(1)
    dosyalar = [f for f in dosyalar if not f.endswith("init.c") and not f.endswith("recovery.c")]
    flags = "-static"
    basari, hata = 0, 0
    for f in dosyalar:
        out = f.replace(".c", "")
...
def create_system_partition():
    ...
    print(Fore.GREEN + "[+] System partition oluşturuluyor...")
    ...
    print(Fore.GREEN + "[+] System partition diske yazıldı.")
...
int main() {
    ...
    int r;
    if (strcmp(bootmode, "recovery") == 0 || strcmp(bootmode, "fastboot") == 0)
    ...
    with open("/tmp/initramfs_init.c", "w") as f:
        f.write(code)
    ret = os.system(f"{gcc_loc} -static /tmp/initramfs_init.c -o rootfs/init")
    if ret == 0:
        print(Fore.GREEN + "[+] Minimal init derlendi -> rootfs/init")
    else:
        print(Fore.RED + "[-] Init derlenemedi.")
        sys.exit(1)
...
def create_recovery_partition():
    print(Fore.GREEN + "[+] Recovery partition oluşturuluyor...")
    recovery_init = tempfile.NamedTemporaryFile(delete=False, suffix=".recovery-init")
    recovery_init.close()
    ret = os.system(f"{gcc_loc} -static pati-commands/recovery.c -o {recovery_init.name}")
    if ret != 0:
        print(Fore.RED + "[-] Recovery init derlenemedi.")
        os.unlink(recovery_init.name)
        sys.exit(1)
    print(Fore.GREEN + "[+] Recovery init derlendi.")
    recovery_dir = tempfile.mkdtemp()
    os.makedirs(f"{recovery_dir}/sbin", exist_ok=True)
    os.makedirs(f"{recovery_dir}/bin", exist_ok=True)
    os.makedirs(f"{recovery_dir}/dev", exist_ok=True)
    os.system(f"cp {recovery_init.name} {recovery_dir}/sbin/init")
    os.system(f"cp rootfs/bin/shell {recovery_dir}/bin/shell")
    if os.path.exists("rootfs/lib/paticommands/mkfs"):
        os.system(f"cp rootfs/lib/paticommands/mkfs {recovery_dir}/bin/mkfs")
    if os.path.exists("rootfs/lib/paticommands/tar"):
        os.system(f"cp rootfs/lib/paticommands/tar {recovery_dir}/bin/tar")
    os.system(f"chmod +x {recovery_dir}/sbin/init {recovery_dir}/bin/shell {recovery_dir}/bin/mkfs {recovery_dir}/bin/tar")
    for dev in [("console", "c", 5, 1), ("null", "c", 1, 3), ("ttyAMA0", "c", 204, 64), ("tty", "c", 5, 0)]:
        os.system(f"sudo mknod {recovery_dir}/dev/{dev[0]} {dev[1]} {dev[2]} {dev[3]}")
    recovery_img = tempfile.NamedTemporaryFile(delete=False, suffix=".ext4")
    recovery_img.close()
    subprocess.run(["dd", "if=/dev/zero", f"of={recovery_img.name}", "bs=1M", "count=32"], check=True, capture_output=True)
    subprocess.run(["genext2fs", "-b", "32768", "-d", recovery_dir, recovery_img.name], check=True)
    subprocess.run(["dd", f"if={recovery_img.name}", "of=pati_disk.img", "bs=1M", "seek=454", "count=32", "conv=notrunc"], check=True)
    os.unlink(recovery_img.name)
    os.system(f"rm -rf {recovery_dir}")
    print(Fore.GREEN + "[+] Recovery partition diske yazıldı.")
